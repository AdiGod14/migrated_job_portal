...
interface Job {
  _id?: string;
  id?: string; // For compatibility with some job objects
  title: string;
  employerId: string;
  employerName: string;
  domain: string;
...
export class MyListingsComponent implements OnInit {
  ...
  openModal(job?: Job) {
    ...
    if (job) {
      this.selectedJobId = job._id || job.id || null;
  ...
  saveJob() {
    ...
    console.log('EditMode:', this.isEditMode, 'SelectedJobId:', this.selectedJobId);
