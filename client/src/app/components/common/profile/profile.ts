  ...
  fetchData(): void {
    ...
    if (this.role === 'user') {
      this.userService.getUserData(this.userId!).subscribe({
        next: (res: any) => {
          this.userOrEmployer = res.user || res;
    ...
    update$.subscribe({
      next: (res: any) => {
        ...
        if (typeof window !== 'undefined') {
            if (this.role === 'user' && res.user) {          
              const updatedUser = {
            ...res.user,
              _id: res.user._id || res.user.id || this.userId, // Preserve ID
              id: res.user.id || res.user._id || this.userId   // Ensure both formats
            };
          localStorage.setItem('user', JSON.stringify(updatedUser));
            } else if (this.role === 'employer' && res.employer) { 
                  const updatedEmployer = {
                ...res.employer,
                _id: res.employer._id || res.employer.id || this.userId,
                id: res.employer.id || res.employer._id || this.userId
              };
              localStorage.setItem('user', JSON.stringify(updatedEmployer));
            }
        }
      },
