...
export class DashboardComponent implements OnInit {
  ...
  //       this.appliedJobs = jobs.map((job: any) => ({
  //       ...job,
  //       statusText: statusMap[job.status as number] || 'Unknown',
  //     }));
  ...
  this.jobService.getJobSummaries(this.employerId!, this.token!).subscribe({
    next: (res: any) => {
      const jobs = res.jobs ?? []; // extract the jobs array
      ...
      this.employerBarChartOptions = {
        ...
        yAxis: {
          type: 'category',
          data: jobs.map((job: any) => job.title),
          axisLabel: {
        ...
        series: [
          {
            type: 'bar',
            data: jobs.map((job: any) => job.applicantCount),
